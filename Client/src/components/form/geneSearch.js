...
const GeneSearch = props => {
    ...
    const handleBarSelections = (data, e) => {
        if (e.action === 'select-option')
            setGenes([...selectedGenes, e.option.value])
        else if (e.action === 'remove-value')
            setGenes(selectedGenes.filter(x => x !== e.removedValue))
    ...
    const handleSearchChange = (value) => {
        ...
        if (value !== '') {
            ...
            fetch(url(value))
                .then(res => res.json())
                .then(summary => summary.hits.map(({symbol, name}, i) =>
                    ({key: i, description: name, name: symbol})))
