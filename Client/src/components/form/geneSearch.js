...
const GeneSearch = (props) => {
  ...
  const handleBarSelections = (data, e) => {
    if (e.action === 'select-option') { setGenes([...selectedGenes, e.option.value]) } else if (e.action === 'remove-value') { setGenes(selectedGenes.filter((x) => x !== e.removedValue)) } else if (e.action === 'clear') { setGenes([]) }
  }
